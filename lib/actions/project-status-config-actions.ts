...
// Helper to check if name_th column exists in project_status_configs table
async function checkStatusConfigNameThColumn(pool: any): Promise<boolean> {
    const columnCheck = await pool.request().query(`
        SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = 'pms' AND TABLE_NAME = 'project_status_configs'
...
export async function getProjectStatusConfigs() {
    try {
        ...
        const hasNameTh = await checkStatusConfigNameThColumn(pool)

        const selectCols = hasNameTh
            ? `id, code, name, name_th, color, is_final, sort_order, is_active, created_at`
            : `id, code, name, NULL as name_th, color, is_final, sort_order, is_active, created_at`

        const result = await pool.request()
            .query(`
                SELECT ${selectCols} FROM pms.project_status_configs
...
export async function createProjectStatusConfig(data: Omit<ProjectStatusConfig, 'id'>) {
    try {
        ...
        if (hasNameTh) {
            ...
            await request.query(`
                INSERT INTO pms.project_status_configs (code, name, color, is_final, sort_order)
                VALUES (@code, @name, @color, @is_final, @sort_order)
            `)
        }
