    ...
    return (
        <div
            ...
            className={cn(
                ...
                task.isLocked ? "opacity-60 cursor-not-allowed" : "cursor-grab active:cursor-grabbing hover:shadow-md"
            )}
        ...
        >
            ...
            {!task.isLocked && onUnassign && (
                <button
                    ...
                    className="absolute -top-2 -right-2 w-5 h-5 bg-red-600 hover:bg-red-700 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center shadow-md z-10"
                    title="ถอนงาน"
                >
                    ×
                </button>
...
export function GanttResourceView() {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        const [employeeId, dateStr] = overId.split(':')

        if (!employeeId || !dateStr) return

        // Check if dropping on the same location (skip if task is already assigned to this employee on this date)
        const activeData = active.data.current
        if (activeData?.employeeId === employeeId && activeData?.date === dateStr) {
            return // No change needed
        }

        const toastId = toast.loading("กำลัง Assign งาน...")

        const result = await reassignTask(taskId, employeeId, dateStr)

        if (result.success) {
            toast.success("Assign งานสำเร็จ", { id: toastId })
            if (result.warning) {
            ...
            toast.error(result.error || "Assign งานล้มเหลว", { id: toastId })
        }
    }
            ...
            isFullscreen && "fixed inset-0 z-50 rounded-none"
                ...
                <div className="px-4 py-3 border-b flex items-center justify-between shrink-0 bg-slate-50">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={loadData} className="p-1.5 hover:bg-slate-100 rounded-lg border text-slate-500">
                            <RefreshCw className={cn("w-4 h-4", isLoading && "animate-spin")} />
                        </button>

                        <button
                            onClick={() => setIsFullscreen(!isFullscreen)}
                            className="p-1.5 hover:bg-slate-100 rounded-lg border text-slate-500"
                        >
                ...
                <div className="flex-1 overflow-auto" ref={containerRef}>
                    {isLoading ? (
                        ...
                        <div className="min-w-max">
                            {/* Header Row */}
                            <div className="flex border-b bg-slate-50 sticky top-0 z-20" style={{ height: HEADER_HEIGHT }}>
                                {/* Employee Column Header */}
                                <div className="sticky left-0 z-30 bg-slate-50 border-r shadow-sm flex items-center px-3" style={{ width: EMPLOYEE_COL_WIDTH }}>
                                    ...
                                    {dates.map(date => {
                                        ...
                                        return (
                                            <div
                                                key={dateStr}
                                                className={cn(
                                                    "border-r flex items-center justify-center font-medium text-sm",
                                                    isToday ? "bg-blue-600 text-white" : "text-slate-700"
                                                )}
                                                style={{ width: totalDayWidth, height: HEADER_HEIGHT }}
                                            >
                                                {date.toLocaleDateString('th-TH', { weekday: 'short' })} {date.getDate()}
                                                <span className="ml-1 text-xs opacity-70">({hours.length}h)</span>
                                            </div>
                                        )
                                    })}
                                </div>
                            ...
                            {filteredEmployees.map(emp => (
                                <div key={emp.employee_id} className="flex border-b hover:bg-slate-50/50">
                                    {/* Employee Info - Fixed */}
                                    <div
                                        className="sticky left-0 z-10 bg-white border-r shadow-sm flex items-center gap-3 px-3"
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                                ...
                                                <span className={cn(
                                                    "text-xs font-medium",
                                                    emp.average_workload_percent > 100 ? "text-red-500" :
                                                        emp.average_workload_percent > 80 ? "text-amber-500" : "text-green-600"
                                                )}>
                                                    {emp.average_workload_percent}%
                                                </span>
                                            </div>
                                            <div className="text-[10px] text-slate-400 mt-0.5">
                                                {emp.total_assigned_hours}h assigned
                                            </div>
                                        </div>
                                    </div>
        ...
        >
            ...
            <DragOverlay dropAnimation={null}>
                {activeTask && (
                    <div
                        className={cn(
                            "h-10 rounded-md border-2 flex items-center px-2 text-white text-xs font-medium shadow-lg",
                    ...
                    >
                        <span className="font-bold mr-1">{activeTask.projectCode}</span>
                        <span className="truncate">{activeTask.title}</span>
                        <span className="shrink-0 ml-1 bg-white/20 px-1 rounded text-[10px]">
                            {activeTask.hours}h
                        </span>
                    </div>
                )}
