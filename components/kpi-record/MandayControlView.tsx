...
import { CheckCircle2, XCircle, RefreshCw, Calendar, Target, Users, FolderKanban, BarChart3, Calculator, Info, ExternalLink } from "lucide-react"
import {
    getMandayControlKPI,
...
// Gauge Chart Component - Light Theme
function GaugeChart({ value, size = 160 }: { value: number; size?: number }) {
    const r = 60, cx = size / 2, cy = size / 2 + 5
    ...
    return (
        <svg width={size} height={size - 20} viewBox={`0 0 ${size} ${size - 20}`}>
            <path d={arc(sA, eA)} fill="none" stroke="#e2e8f0" strokeWidth={12} strokeLinecap="round" />
            {v > 0 && <path d={arc(sA, vA)} fill="none" stroke={color} strokeWidth={12} strokeLinecap="round" />}
            <line x1={tp.x} y1={tp.y - 8} x2={tp.x} y2={tp.y + 8} stroke="#f59e0b" strokeWidth={2} strokeLinecap="round" />
            <text x={tp.x} y={tp.y - 12} textAnchor="middle" fill="#f59e0b" fontSize="8" fontWeight="600">85%</text>
            <text x={cx} y={cy - 4} textAnchor="middle" fill="#1e293b" fontSize="24" fontWeight="800">{value.toFixed(1)}%</text>
            <text x={cx} y={cy + 12} textAnchor="middle" fill={color} fontSize="10" fontWeight="700">{getStatusLabel(value)}</text>
        </svg>
    )
}

// Progress Bar Component
function ProgressBar({ value, height = 6, showLabel = false }: { value: number; height?: number; showLabel?: boolean }) {
    ...
    return (
        <div className="relative">
            ...
            {showLabel && <span className="text-[9px] text-slate-500 mt-0.5 block text-center">{value}%</span>}
        </div>
    )
}

// Weight Bar Component - Dynamic based on unique milestones
...
// Sparkline Component - Light Theme
...
export default function MandayControlView({ embedded = false }: MandayControlViewProps) {
    ...
    // Get score explanation based on ratio
    ...
    const uniqueMilestones = useMemo(() => {
        ...
        // Collect all unique milestones with their default weights
        ...
        <div className={embedded ? "p-4 space-y-4" : "p-6 space-y-6 w-full bg-slate-50 min-h-screen"}>
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <div className="text-xs space-y-1">
                                    <div className="flex justify-between"><span className="text-emerald-600">Actual ≤ Plan</span><span className="font-bold">100%</span></div>
                                    <div className="flex justify-between"><span className="text-yellow-600">Actual ≤ Plan×1.1</span><span className="font-bold">90%</span></div>
                                    <div className="flex justify-between"><span className="text-orange-600">Actual ≤ Plan×1.2</span><span className="font-bold">70%</span></div>
                                    <div className="flex justify-between"><span className="text-rose-600">Actual &gt; Plan×1.2</span><span className="font-bold">50%</span></div>
                                </div>
                            </div>
                            <div>
                                <h5 className="font-semibold text-slate-700 text-sm mb-2">Milestone Weights (ตัวอย่าง)</h5>
                                <WeightBar milestones={uniqueMilestones.length > 0 ? uniqueMilestones : [
                                    { name: "Mapping Data", weight: 30 },
                                    { name: "System Test", weight: 30 },
                                    { name: "User Acceptance Test", weight: 20 },
                                    { name: "Go-Live", weight: 10 },
                                    { name: "Close Go-Live", weight: 10 },
                                ]} />
                                <div className="text-[9px] text-slate-500 mt-1 text-center">* Weight แตกต่างกันตามโปรเจค</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <select
                    ...
                    className="px-3 py-2 border border-slate-200 rounded-lg focus:border-orange-500 outline-none bg-white font-medium text-sm"
                >
                ...
                <button onClick={() => setMonthFilter("all")} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${monthFilter === "all" ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    ทั้งปี
                </button>
                {activeMonths.map(m => (
                    <button key={m.value} onClick={() => setMonthFilter(m.value.toString())} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${monthFilter === m.value.toString() ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                        {m.label}
                    </button>
                ))}
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className={`rounded-xl shadow-sm p-4 border-2 ${summary.isPass ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
                        ...
                        <div className="flex justify-center">
                            <GaugeChart value={overallKPI} size={140} />
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl bg-blue-50 border border-blue-200 shadow-sm p-4">
                        <div className="flex items-center gap-2 text-blue-600 mb-2">
                            <Users size={16} />
                            <span className="text-xs font-semibold">แผน (Plan)</span>
                        </div>
                        <div className="text-3xl font-black text-blue-600">{totalManday.toLocaleString()}</div>
                    ...
                    <div className="rounded-xl bg-amber-50 border border-amber-200 shadow-sm p-4">
                        <div className="flex items-center gap-2 text-amber-600 mb-2">
                            <Calculator size={16} />
                            <span className="text-xs font-semibold">จริง (Actual)</span>
                        </div>
                        <div className="text-3xl font-black text-amber-600">{(summary.totalActualMandays || 0).toLocaleString()}</div>
                        <div className="text-xs text-amber-500">Man-days</div>
                    </div>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-center">
                                ...
                                <div className="text-[10px] text-rose-600">ไม่ผ่าน</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                data.map((row, idx) => {
                                    ...
                                    // Get actual milestone data for this project
                                    ...
                                    return (
                                        <tr key={idx} className={`border-b border-slate-100 hover:bg-slate-50 ${row.is_pass !== 1 ? 'bg-rose-50/30' : ''}`}>
                                            ...
                                            {uniqueMilestones.map((ms) => {
                                                ...
                                                // If project doesn't have this milestone, show empty
                                                if (!msData) {
                                                    return (
