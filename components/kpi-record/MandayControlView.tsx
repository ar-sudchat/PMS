...
import { CheckCircle2, XCircle, RefreshCw, Calendar, Target, FolderKanban, BarChart3, Calculator, Info, ExternalLink } from "lucide-react"
import {
    getMandayControlKPI,
...
function ProgressBar({ value, height = 6 }: { value: number; height?: number }) {
    const c = value >= 100 ? "#10b981" : value >= 85 ? "#3b82f6" : value >= 70 ? "#f59e0b" : "#ef4444"
    return (
...
function Sparkline({ data, width = 100, height = 28 }: { data: { month: number; value: number }[]; width?: number; height?: number }) {
    ...
    const color = last.value >= 85 ? "#10b981" : last.value >= 70 ? "#f59e0b" : "#ef4444"
    const tY = height - 4 - (85 / 100) * (height - 8)
    return (
        <svg width={width} height={height} className="block">
            <line x1={4} y1={tY} x2={width - 4} y2={tY} stroke="#f59e0b40" strokeWidth={1} strokeDasharray="2,2" />
...
export default function MandayControlView({ embedded = false }: MandayControlViewProps) {
    ...
    const getScoreExplanation = (planned: number | null, actual: number | null) => {
        ...
        if (ratio <= 1.1) return { label: `Actual <= Plan x1.1 (${(ratio * 100).toFixed(0)}%) -> 90%`, color: 'text-yellow-600' }
        if (ratio <= 1.2) return { label: `Actual <= Plan x1.2 (${(ratio * 100).toFixed(0)}%) -> 70%`, color: 'text-orange-600' }
        return { label: `Actual > Plan x1.2 (${(ratio * 100).toFixed(0)}%) -> 50%`, color: 'text-rose-600' }
    }
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-orange-100 text-sm mt-1">ควบคุม Manday ตาม Milestone (due date) — Target &gt;= 85%</p>
                            </div>
                        </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <h5 className="font-semibold text-slate-700 text-sm mb-2">เกณฑ์ให้คะแนน (Actual vs Plan)</h5>
                                <div className="text-xs space-y-1">
                                    ...
                                    <div className="flex justify-between"><span className="text-yellow-600">Actual &lt;= Plan x 1.1</span><span className="font-bold">90%</span></div>
                                    <div className="flex justify-between"><span className="text-orange-600">Actual &lt;= Plan x 1.2</span><span className="font-bold">70%</span></div>
                                    <div className="flex justify-between"><span className="text-rose-600">Actual &gt; Plan x 1.2</span><span className="font-bold">50%</span></div>
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="bg-slate-50 p-2 rounded text-xs font-mono text-center border">
                                    <div>Sigma (คะแนน x น้ำหนัก)</div>
                                    <div className="border-t border-slate-300 my-1"></div>
                                    <div>Sigma น้ำหนัก</div>
                                </div>
                                <div className="text-[9px] text-slate-500 mt-1 text-center">* คิดตาม due date ของ Milestone</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {summary && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="rounded-xl bg-emerald-50 border border-emerald-200 shadow-sm p-4">
                        ...
                        <div className="text-xs text-emerald-500">&gt;= 85%</div>
                    </div>
                    ...
                    <div className="rounded-xl bg-rose-50 border border-rose-200 shadow-sm p-4">
                        ...
                        <div className="text-xs text-rose-500">&lt; 85%</div>
                    </div>
                </div>
            )}
                    ...
                    <table className="w-full">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="text-center px-2 py-3 text-xs font-semibold text-slate-700 w-20">
                                    <div>MD</div>
                                    <div className="text-[9px] text-slate-400 font-normal">Plan/Actual</div>
                                </th>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="text-xs text-slate-500 bg-slate-50 rounded p-3 border border-slate-200">
                                ...
                                <div className="grid grid-cols-2 gap-1">
                                    ...
                                    <div className="text-yellow-600">Actual &lt;= Plan x 1.1</div><div className="text-right font-medium">90%</div>
                                    <div className="text-orange-600">Actual &lt;= Plan x 1.2</div><div className="text-right font-medium">70%</div>
                                    <div className="text-rose-600">Actual &gt; Plan x 1.2</div><div className="text-right font-medium">50%</div>
                                </div>
                            </div>
                        </div>
                    )}
